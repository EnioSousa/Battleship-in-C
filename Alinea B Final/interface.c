...
void printShipActive(Player* p){
	...
	printf(ANSI_COLOR_RED "Active enemy: " ANSI_COLOR_RESET);
	for(int i=0;i<5;i++){
	 if(p->map->ship[i].active>0)
	 printf(ANSI_COLOR_GREEN "%s active: %d " ANSI_COLOR_RESET, p->map->ship[i].name,p->map->ship[i].active);
	 }
         printf("\n\n");
}
